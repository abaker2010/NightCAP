    ...
    def update(
        ...
        for _upditem in updatetable.all():
            ...
            if updaterrule == NightcapCoreUpaterRules.Module:
                _found = self._find_module_item(
                    localtable, _upditem[checkonrow], checkonrow
                )
            elif updaterrule == NightcapCoreUpaterRules.Submodule:
                _found = self._find_submodule_item(
                    localtable=localtable,
                    find=_upditem[checkonrow],
                    findtwo=_upditem[checkonrowtwo],
                    checkonrow=checkonrow,
                    checkonrowtwo=checkonrowtwo,
                )
                ...
                if _found == []:
                    if updaterrule == NightcapCoreUpaterRules.Package:
                        self.printer.print_formatted_additional(
                            text=str(_upditem[checkonrow][checkonrowtwo]), leadingTab=3
                        )
                        self._insert(localtable, _upditem)
                    elif updaterrule == NightcapCoreUpaterRules.Module:
                        self.printer.print_formatted_additional(
                            text=str(_upditem[checkonrow]), leadingTab=3
                        )
                        self._insert(localtable, _upditem)
                    elif updaterrule == NightcapCoreUpaterRules.Submodule:
                        self.printer.print_formatted_additional(
                            text=str(
                                _upditem[checkonrow] + " / " + _upditem[checkonrowtwo]
                            ),
                            leadingTab=3,
                        )
                        self._insert(localtable, _upditem)
                    ...
                    if updaterrule == NightcapCoreUpaterRules.Package:
                        self.printer.print_formatted_check(
                            text=str("UID: " + _upditem[checkonrow][checkonrowtwo]),
                            leadingTab=3,
                        )
                    else:
                        self.printer.print_formatted_check(
                            text=_upditem[checkonrow], leadingTab=3
                        )
            except Exception as e:
