...
class NightcapUpdater:
    ...
    def __get_update(self):
        ...
        if self.isMainBranch:
            ...
            resp = requests.get(
                "https://github.com/abaker2010/NightCAP/archive/Dev.zip", stream=True
            )
            self.tmpUpdateLocation = os.path.join(self.tmpdir, "NightCAP-dev")
        total = int(resp.headers.get("content-length", 0))

        description = (
            Fore.LIGHTMAGENTA_EX + "[-] Using main branch: update.zip"
            if self.isMainBranch
            ...
            try:
                ...
                #         print("*"*20,"\n")
                    ...
                    self.printer.print_formatted_check(
                        "Skipping: Mongo Updater Needed", leadingTab=3
                    )
                ...
                #         print("Skipping DB:", "Submodules")
                    ...
                    self.printer.print_formatted_check(
                        "Skipping: Mongo update needed", leadingTab=3
                    )
                    ...
                    raise Exception(
                        "Error with file" + str(udb).replace(self.tmpdir, "Tmp -> ")
                    )
                    # try:
