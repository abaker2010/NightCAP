...
class NightcapMongoHelper:
    ...
    def change_mongo_server(self, ip: str = None, port: str = None):
        ...
        if('127.0.0.1' or 'localhost' in self.conf.currentConfig["MONGOSERVER"]["ip"]):
            ...
            self.printer.print_underlined_header_undecorated(text="MongoDB", leadingTab=2)
            if _docker_checker.mongo_im_exists == False:
                self.printer.print_formatted_delete(text='Container Status', optionaltext=self.docker_helper.get_mongo_container_status(), leadingTab=3,)
                self.printer.print_formatted_delete(text="Mongo", optionaltext="Missing", leadingTab=3, endingBreaks=1)
            else:
                self.printer.print_formatted_check(text='Container Status', optionaltext=self.docker_helper.get_mongo_container_status(), leadingTab=3, textColor=Fore.CYAN)
                self.printer.print_formatted_additional(text="Mongo Image", optionaltext="Exists", leadingTab=3, optionalTextColor=Fore.RED, endingBreaks=1)


            self.printer.print_underlined_header_undecorated(text="Nighcapsite", leadingTab=2)
            if _docker_checker.ncs_exits == False:
                self.printer.print_formatted_delete(text='Container Status', optionaltext=self.docker_helper.get_site_container_status(), leadingTab=3,)
                self.printer.print_formatted_delete(text="Nightcapsite", optionaltext="Missing", leadingTab=3, endingBreaks=1)
            else:
                self.printer.print_formatted_check(text='Container Status', optionaltext=self.docker_helper.get_site_container_status(), leadingTab=3, textColor=Fore.CYAN)
                self.printer.print_formatted_additional(text="Nightcapsite", optionaltext="Exists", leadingTab=3, optionalTextColor=Fore.RED, endingBreaks=1)
        
            self.printer.print_underlined_header(text="Error connecting to mongo server")
            self.printer.print_formatted_additional(text="Initialize", optionaltext="I", leadingTab=2, optionalTextColor=Fore.MAGENTA)
            self.printer.print_formatted_additional(text="(Re)Start", optionaltext="R | S", leadingTab=2, optionalTextColor=Fore.MAGENTA)
            self.printer.print_formatted_additional(text="Change", optionaltext="C", leadingTab=2, optionalTextColor=Fore.MAGENTA, endingBreaks=1)

            _selection = input(Fore.LIGHTGREEN_EX + "\t\tOption: " + Style.RESET_ALL).lower()

            if _selection.strip().lower() == "c":
                yes = input(Fore.YELLOW + "\t\tAre you sure you want to change the settings (Y/N): " + Style.RESET_ALL).lower()
                self._change_mogo_settings(yes)
                ScreenHelper().clearScr()
                self.printer.print_formatted_additional(text='Rebooting...')
                os.execv(sys.argv[0], sys.argv)
            elif _selection.strip().lower() == 'i':
                yes = input(Fore.YELLOW + "\t\tAre you sure you want to initialize containers (Y/N): " + Style.RESET_ALL).lower()
                ...
                yes = input(Fore.YELLOW + "\t\tAre you sure you want to start containers (Y/N): " + Style.RESET_ALL).lower()
                if yes in self.yes:
                    self.docker_helper.prepare_containers()
                    self.printer.print_formatted_additional(text='Rebooting...')
                    os.execv(sys.argv[0], sys.argv)
