...
class NightcapSimpleReport(NightcapCoreReportBase):
    ...
    def save(self):
        ...
        if self.params["project"] == None:
            ...
            print("---------------")
            print(
                NightcapPaths().generate_path(
                    NightcapPathsEnum.Reporting,
                    [self.params["project"]["project_number"]]
                    + self.spath.split(os.sep)[:-1]
                    + ["index.html"],
                )
            )

            report_files = NightcapCoreFiles(
                NightcapPaths().generate_path(
                    NightcapPathsEnum.Reporting,
                    [self.params["project"]["project_number"]]
                    + self.spath.split(os.sep)[:-1]
            ...
            if (
                len(self.custom_style_sheets) == 0
                or self.replace_original_style_sheet == False
            ):
                print("Second Style Sheet Is Not Added")
                _html.append(
                    '<link rel="stylesheet" href="http://localhost:8056/style.css">'
                )
                ...
                try:
                    ...
                    for sheet in self.custom_style_sheets:
                        _html.append(
                            custom_style_format
                            % (
                                base64.b64encode((sheet).encode("ascii")).decode(
                                    "utf-8"
                                )
                            )
                        )
            ...
            _html.append(
                '<div class="report w3-container w3-center w3-round w3-light-grey">'
            )
            _html.append(
                "<h1>Scan Used: %s</h1>" % os.sep.join(self.spath.split(os.sep)[:-1])
            )
            ...
            for k in self.data.keys():
                _html.append("<div>")
                _html.append(
                    (self.header_formats[self.data[k].format] % self.data[k].name)
                )
