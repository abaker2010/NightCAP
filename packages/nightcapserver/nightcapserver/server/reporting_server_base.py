...
class NightcapCoreServerReportingBase(NighcapCoreCLIBaseConfiguration):
    ...
    def get_route(self, route):
        if route == "/":
            ...
            return self._generate_standard_template_page(
                self.basepath, self._generate_home_page(_path)
            )
        ...
        if "/project/" in route:
            ...
            if len(subd) <= 3:
                return self._generate_standard_template_page(
                    self.basepath, self._generate_page(route, subd)
                )
            elif len(subd) == 4:
                return self._generate_report_template_page(
                    self.basepath, self._generate_reports_page(self.basepath, subd)
                )
            else:
                pass
    ...
    def _generate_reports_page(self, basedir: list, subd: list):
        template_file = NightcapPaths().generate_path(
            NightcapPathsEnum.Reporting, subd + ["index.html"]
        )
    ...
    def _generate_home_page(self, path):
        ...
        for l in data:
            if proj_replace in l:
                ...
                print(projs)
                for proj in projs:
                    htm = "<a href='/project/%s'><li>%s</li></a>" % (
                        proj["project_number"],
                        proj["project_name"],
                    )
                    ndata.append(htm)
    ...
    def _format_list_page(self, header: str, path: str, route: str, opts: list):
        ...
        template_file = NightcapPaths().generate_path(
            NightcapPathsEnum.ReportingTemplates, ["list_template.html"]
        )
        ...
        with open(template_file, "r") as file:
            ...
            for line in lines:
                if list_replacement in line:
                    for opt in opts:
                        op = "<a href='%s'><li>%s</li></a>" % (
                            (route + "/" + opt),
                            str(opt).replace(".html", ""),
                        )
                        t_data.append(op)
