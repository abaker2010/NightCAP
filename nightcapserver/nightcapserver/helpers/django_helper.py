...
class NightcapDjangoDockerHelper(
    ...
    def init_container(self) -> bool:
        try:
            ...
            p = subprocess.Popen(
                ["docker-compose", "-f", _, "up", "--no-start", "website"],
                stdout=DEVNULL,
                stderr=DEVNULL,
            )
            ...
            self.printer.print_formatted_check(
                text="Created Containers", endingBreaks=1
            )
    ...
    def init_image(self) -> bool:
        ...
        if self.image_exists() == NightcapDockerStatus.EXISTS:
            ...
            self.printer.print_formatted_additional(
                "Pulling image please wait...", leadingBreaks=1
            )
            # self.docker.images.pull(self.name+":"+self.tag)
            _ = Path(__file__).resolve().parent.parent
            p = subprocess.Popen(
                ["make", "-C", Path(__file__).resolve().parent.parent.parent],
                stdout=DEVNULL,
                stderr=DEVNULL,
            )
            ...
            if p.returncode != 0:
                self.printer.print_error(
                    Exception("Error pulling docker image: %s" % (p.returncode))
                )
                return False
    ...
    def set_account(self):
        try:
            ...
            try:
                ...
                self.printer.item_3(
                    "Please enter some information to create your web admin account",
                    leadingTab=2,
                    endingBreaks=1,
                )
    ...
    def _set_user(self, _container):
        _username = input(
            Fore.LIGHTGREEN_EX + str("\tUser Name: %s" % (Fore.LIGHTCYAN_EX))
        )
        _email = input(
            Fore.LIGHTGREEN_EX + str("\tEmail Address: %s" % (Fore.LIGHTCYAN_EX))
        )

        if self._email_validation(_email):

            _password = getpass.getpass(Fore.LIGHTGREEN_EX + str("\tPassword:"))
            _retype_password = getpass.getpass(
                Fore.LIGHTGREEN_EX + str("\tRetype Password:")
            )
            if _password == _retype_password:
                _cmd = "".join(
                    [
                        ...
                        (
                            "from django.contrib.auth.models import User; User.objects.create_superuser('%s', '%s', '%s')"
                ...
                self.printer.print_formatted_check(
                    text="Account Created", leadingBreaks=1
                )
                return True
            else:
