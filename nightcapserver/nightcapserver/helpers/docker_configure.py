...
from nightcapcore import ScreenHelper, Printer, NightcapCLIConfiguration, NightcapBanner, Legal
...
class NightcapDockerConfigurationHelper:
    ...
    def check_legal(self):
        ...
        while not self.conf.config.getboolean("NIGHTCAPCORE", "agreement"):
            ...
            agree = self.printer.input("You must agree to our terms and conditions first (y/N)")
    ...
    def env_check(self):
        try:
            print("Django Helper: ", str(self.django_helper.image_exists().value))
            
            if self.mongo_helper.image_exists() ==  NightcapDockerStatus.EXISTS and self.mongo_helper.container_status() ==  NightcapDockerStatus.RUNNING:
                _mongo = (NightcapDockerStatus.EXISTS, NightcapDockerStatus.EXISTS)
            else:
                _mongo = (self.mongo_helper.image_exists(), self.mongo_helper.container_status())
    ...
    def change_configuration(self):
        # self.printer.print_formatted_additional("Configure Docker")
        ...
        self.printer.print_formatted_additional("Note", "Mongo required to be running to start")
        self.printer.print_underlined_header_undecorated(
            "MongoDB", leadingTab=2)
        # if self.mongo_helper.image_exists() != NightcapDockerStatus.MISSING:
        #     print("Mongo Image Exists")
        # else:

        self.printer.item_2(
            text="Image", optionalText=str(self.mongo_helper.image_exists().value).capitalize(), leadingTab=3
        )
        self.printer.item_2(
            text="Container",
            optionalText=str(
                self.mongo_helper.container_status().value).capitalize(),
            leadingTab=3,
        )
        ...
        if _selection != '':
            if _selection.strip().lower() == "i":
                ...
                _selection2 = self.printer.input_return_only("Would you like to start both containers? (B/M/D/E)")
